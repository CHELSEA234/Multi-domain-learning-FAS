...
from config_full_dataset import Config_siwm as Config
from config_full_dataset import Config_siw, Config_oulu
...
def main(args):
	...
	config.desc_str = '_data_'+args.data+'_stage_'+config.phase+\
					  ...
					  '{:.0e}'.format(config.lr)+'_spoof_region_architecture'
	# print(config.desc_str)
	# import sys;sys.exit(0)
	# if args.stage == 'ft':
	# 	config.desc_str += '_simpleB_performance_trueNaive'
	...
	config.tb_dir   = './tb_test_lit_logs'+config.desc_str
	# GX: use os, then permission denied.
	...
	config.csv_file_name  = "./"+config.desc_str+'_epoch_num_'+str(config.epoch_num_pick)+'.csv'

	os.makedirs(config.root_dir, exist_ok=True)
	os.makedirs(config.save_model_dir, exist_ok=True)
	## rename the current folder name.
	...
	# 	os.rename(config.save_model_dir.replace("ft", "pretrain"), config.save_model_dir)
	# 	print("... Folder Renaming is Over...")
	# else:
	# 	print("... Saved-Folder already Exists...")
	print('**********************************************************')
	config.compile_siwm()
	config_siw.compile()
	config_oulu.compile()
...
if __name__ == "__main__":
  ...
  parser.add_argument('--lr', type=float, default=1e-07, help='The starting learning rate.')
  parser.add_argument('--decay_step', type=int, default=1, help='The learning rate decay step.')
  parser.add_argument('--pretrain_folder', type=str, default='./', help='Deprecated function.')
  parser.add_argument('--epoch_num', type=int, default=33, help='To pick the best epoch num.')
